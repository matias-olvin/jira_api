CREATE OR REPLACE TABLE
    `{{ var.value.env_project }}.{{ params['manually_add_pois_dataset'] }}.{{ params['sgplace_visitor_brand_destination_percentage_table'] }}` AS
WITH
    poi_data AS (
        SELECT
            pid AS fk_sgplaces,
    ...
    final_observed_connections AS (
        ...
        WHERE
            agg_weigthed_observed_conns.shared_devices > 0
    )
    -- Formatted Table
SELECT
    poi_to_add AS pid,
                    ...
                    TO_JSON_STRING(
                        ARRAY_AGG(
                            STRUCT (
                                ...
                                CAST(
                                    ROUND(
                                        100 * (shared_devices / total_devices) * (
                                            1 + CAST(REGEXP_EXTRACT(fk_sgbrands, r'(\d)') AS INT64) / 100
                                        ),
                                        2
                                    ) AS STRING
                                ) AS shared_devices
