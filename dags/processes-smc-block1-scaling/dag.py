...
scaling_block_1_bq_api = SourceFileLoader(
    "scaling_block_1_bq_api", f"{steps_path}/scaling_block_1_bq_api.py"
).load_module()
...
with DAG(
    ...
    scaling_block_1_bq_api_end = scaling_block_1_bq_api.register(
        start=start,
        dag=dag,
        env="dev",
        ...
        on_success_callback=callbacks.task_end_custom_slack_alert(
            "U03BANPLXJR",
            msg_title="*POI Visits Scaled Block 1* created using the new models.",
        ),
    )

    scaling_block_1_bq_api_end >> end
