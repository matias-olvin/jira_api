...
destinations_percentages as (
    ...
    from (
        ...
        inner join
          `{{ var.value.env_project }}.{{ params['public_feeds_staging_dataset'] }}.{{ params['postgres_batch_dataset'] }}-{{ params['sgbrandraw_table'] }}-{{ var.value.data_feed_data_version.replace('.', '-') }}` as brands
...
from  -- Ensure we provide exact same stores in visitors and visits feeds: postgres
    `{{ var.value.env_project }}.{{ params['public_feeds_staging_dataset'] }}.{{ params['postgres_batch_dataset'] }}-{{ params['sgplaceraw_table'] }}-{{ var.value.data_feed_data_version.replace('.', '-') }}` as places
...
where
    monthly_visits.local_date >= DATE("{{ params['backfill_start_date'] }}")
