...
with DAG(
    ...
    insert_store_visitors_into_temp = BigQueryInsertJobOperator(
        task_id="insert_store_visitors_into_temp",
        configuration={
            "query": {
                "query": "{% include './bigquery/store_visitors_temp.sql' %}",
                "useLegacySql": "false",
            }
        },
        params=dag.params,
    ...
    create_snapshot_monthly_update >> insert_store_visitors_into_temp
    ...
    insert_store_visitors_into_temp >> data_quality_start
    ...
    DATAPROC_CLUSTER_NAME=f"pby-product-p-dpc-euwe1-acc-export-store-vstrs"
    ...
    with TaskGroup(group_id="data-feed-export-backfill-finance") as feed_export_finance:
        ...
        configuration={
            "query": {
                "query": "{% include './bigquery/store_visitors_finance_backfill.sql' %}",
                "useLegacySql": "false",
            }
        },
        params=dag.params,
    ...
    clear_store_visitors_for_versioning = GCSDeleteObjectsOperator(
            task_id="clear_store_visitors_for_versioning",
            bucket_name="{{ params['versioning_bucket'] }}",
            prefix="type-1/{{ params['store_visitors_table'] }}/{{ next_ds.replace('-', '/') }}/",
    )

    export_store_visitors_for_versioning = OlvinBigQueryOperator(
        task_id="export_store_visitors_for_versioning",
        query="{% include './bigquery/export_store_visitors_for_versioning.sql' %}",
        billing_tier="high",
    )

    assert_volume >> create_export_to_gcs_cluster >> feed_export >> feed_export_finance >> delete_cluster >> end

    feed_export >> clear_store_visitors_for_versioning >> export_store_visitors_for_versioning
