...
def register(start: TaskInstance, dag: DAG) -> TaskInstance:
    ...
    trigger_regressors_group_visits_dictionary = BashOperator(
        ...
        f'dags trigger -- -e "{{{{ ds }}}} {datetime.now().time()}" {dag.params["dag-time_factors_group_residuals_dictionary"]} '
        "|| true ",  # this line is needed due to gcloud bug.
        dag=dag,
    )

    wait_for_time_factors_group_residuals_dictionary = wait_for_sns_dag_run_completion(
        dag.params["dag-time_factors_group_residuals_dictionary"]
    )
    ...
    (
        ...
        >> wait_for_time_factors_group_residuals_dictionary
        >> copy_task
    )
