...
def register(start: TaskInstance, dag: DAG) -> TaskInstance:
    ...
    with TaskGroup(group_id="trigger_cameo_profiles_backfill") as group:
        # Triggering the cameo_profiles_backfill_pipeline.
        trigger = TriggerDagRunOperator(
            task_id=f"trigger_{dag.params['cameo_profiles_backfill_pipeline']}",
            trigger_dag_id="{{ params['cameo_profiles_backfill_pipeline'] }}",
            execution_date=f"{datetime.today().strftime('%Y-%m-%d')}",
            reset_dag_run=True,
        )
        # Waiting for the cameo_profiles_backfill_pipeline to finish.
        sensor = ExternalTaskSensor(
            task_id=f"{dag.params['cameo_profiles_backfill_pipeline']}_sensor",
            external_dag_id=f"{dag.params['cameo_profiles_backfill_pipeline']}",
            poke_interval=60 * 60,
            mode="reschedule",
            timeout=60 * 60 * 24 * 30,
        ...
        task_to_fail = MarkSuccessOperator(
            task_id="check_cameo_profiles_backfill_end",
        )
