DELETE `{{ var.value.env_project }}.{{ params['smc_metrics_dataset'] }}.{{ params['time_series_analysis_visits_block_groundtruth_table'] }}`
WHERE
    run_date = '{{ ds }}';

INSERT INTO `{{ var.value.env_project }}.{{ params['smc_metrics_dataset'] }}.{{ params['time_series_analysis_visits_block_groundtruth_table'] }}`
with visits_by_group as(
    select
        ...
        CASE 
            ...
            ELSE FALSE
        END AS branded
    from `{{ var.value.env_project }}.{{ params['smc_poi_visits_scaled_dataset'] }}.*` visits
    left join 
