...
with get_visits as (
  ...
  WHERE 
    local_date >= DATE("{{ params['backfill_start_date'] }}")
...
adding_places_columns as (
  ...
  inner join (
    ...
    from
      `{{ var.value.env_project }}.{{ params['public_feeds_staging_dataset'] }}.{{ params['postgres_batch_dataset'] }}-{{ params['sgplaceraw_table'] }}-{{ var.value.data_feed_data_version.replace('.', '-') }}`
  ) using (fk_sgplaces)
  inner join (
    select
      pid as fk_sgbrands,
      name as brand
    from
      `{{ var.value.env_project }}.{{ params['public_feeds_staging_dataset'] }}.{{ params['postgres_batch_dataset'] }}-{{ params['sgbrandraw_table'] }}-{{ var.value.data_feed_data_version.replace('.', '-') }}`
  ) using (fk_sgbrands)
),
