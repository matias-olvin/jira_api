...
def register(
    ...
    with TaskGroup(group_id="data-feed-export-create-tables") as create_tables:
        ...
        data_feed_export_backfill_finance_create_table = OlvinBigQueryOperator(
            task_id="data-feed-export-backfill-finance-create-table",
            query="{% include './bigquery/store_visits_finance_backfill.sql' %}",
            billing_tier="med",
        )

        (
            data_feed_export_backfill_create_table
            >> data_feed_export_backfill_finance_create_table
        )
    ...
    clear_store_visits_for_versioning = GCSDeleteObjectsOperator(
            task_id="clear_store_visits_for_versioning",
            bucket_name="{{ params['versioning_bucket'] }}",
            prefix="type-1/{{ params['store_visits_table'] }}/{{ next_ds.replace('-', '/') }}/",
    )

    export_store_visits_for_versioning = OlvinBigQueryOperator(
        task_id="export_store_visits_for_versioning",
        query="{% include './bigquery/export_store_visits_for_versioning.sql' %}",
        billing_tier="high",
    )
    ...
    create_tables >> clear_store_visits_for_versioning >> export_store_visits_for_versioning
