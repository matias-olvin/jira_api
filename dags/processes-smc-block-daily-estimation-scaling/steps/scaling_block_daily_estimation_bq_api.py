...
def block_daily_estimation_generate_queries(start_date: str, end_date: str, dag: DAG) -> List[str]:
    """
    ...
    for start_date, end_date in split_date_range_list:
        ...
        WHILE d <= '{end_date}' DO
            IF(NOT EXISTS(
                    SELECT local_date
                    FROM `{project}.{dag.params['poi_visits_block_daily_estimation_dataset']}.{year}`
                    WHERE local_date = d)
            ) THEN
            CALL `{dag.params['block_daily_estimation_scaling_procedure']}`(
                "{project}.{dag.params['poi_visits_block_daily_estimation_dataset']}.{year}",
                CAST(d AS STRING),
                "{project}.{dag.params['poi_visits_block_1_dataset']}.{year}",
                "{project}.{dag.params['smc_daily_estimation_dataset']}.{dag.params['daily_factor_table']}"
            );
            END IF;
...
def register(
    ...
    def run_scaling(start_date, end_date, job_id_prefix, env, time_stamp_val, **kwargs):
        ...
        queries = block_daily_estimation_generate_queries(start_date, end_date, dag)
    ...
    job_id_prefix = "smc_block_daily_estimation_scaling"

    now = pendulum.now()
    formatted_time = now.format("mm_ss")

    run_block_daily_estimation_scaling_queries = PythonOperator(
        task_id="run_block_daily_estimation_scaling_queries",
    ...
    check_status_of_run_block_daily_estimation_scaling_queries = PythonOperator(
        task_id="check_status_of_run_block_daily_estimation_scaling_queries",
    ...
    start >> run_block_daily_estimation_scaling_queries >> check_status_of_run_block_daily_estimation_scaling_queries

    return check_status_of_run_block_daily_estimation_scaling_queries
