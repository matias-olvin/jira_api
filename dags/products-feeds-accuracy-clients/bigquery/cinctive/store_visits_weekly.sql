EXPORT DATA OPTIONS(
  uri="gs://{{ params['feeds_staging_gcs_no_pays_bucket'] }}/cinctive/export_date={{ next_ds.replace('-', '') }}/{{ params['store_visits_table'].replace('_','-') }}/weekly/*.csv.gz",
  format='CSV',
  compression='GZIP',
  overwrite=true,
  header=true) AS
...
WHERE 
  week_starting < DATE_TRUNC("{{ next_ds }}", WEEK)
...
-- and then 60 days
  ...
  AND week_starting >= DATE_TRUNC("{{ next_ds }}", WEEK);