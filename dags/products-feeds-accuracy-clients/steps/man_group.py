...
def register(start: TaskInstance, dag: DAG, env: str) -> TaskGroup:
    ...
    def branch_task_first_monday_fn(**context):
        ...
        return (
            "man_group.clear_gcs_placekeys_monthly"
            if run_date == first_monday
            else "man_group.end"
        )

    dataproc_cluster_name = (
        "pby-product-d-dpc-man-group-send"
        if env == "dev"
        else "pby-product-p-dpc-man-group-send"
    )

    with TaskGroup(group_id="man_group") as group:
        ...
        clear_s3_store_visits_trend = S3DeleteObjectsOperator(
            ...
            prefix="export_date={{ next_ds.replace('-', '') }}/{{ params['store_visits_trend_table'].replace('_','-') }}/",
            aws_conn_id="s3_conn",
        )
        ...
        clear_s3_store_visitors = S3DeleteObjectsOperator(
            ...
            bucket="{{ params['feeds_staging_aws_bucket_man_group'] }}",
            prefix="export_date={{ next_ds.replace('-', '') }}/{{ params['store_visitors_table'].replace('_','-') }}/",
            aws_conn_id="s3_conn",
        )
        ...
        clear_s3_placekeys = S3DeleteObjectsOperator(
            ...
            bucket="{{ params['feeds_staging_aws_bucket_man_group'] }}",
            prefix="export_date={{ next_ds.replace('-', '') }}/{{ params['placekeys_table'].replace('_','-') }}/",
            aws_conn_id="s3_conn",
        )

        source_uri = "gs://{{ params['feeds_staging_gcs_no_pays_bucket'] }}/man_group/export_date={{ next_ds.replace('-', '') }}"
        dest_uri = "s3a://{{ params['feeds_staging_aws_bucket_man_group'] }}/export_date={{ next_ds.replace('-', '') }}"
