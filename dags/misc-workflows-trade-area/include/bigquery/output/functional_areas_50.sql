CREATE OR REPLACE TABLE `{{ var.value.env_project }}.{{ params['trade_area_dataset'] }}.{{ params['functional_areas_50_table'] }}` -- table with the 30 trade areas
...
polygon
FROM base_table
WHERE coverage_target = 0.5 AND polygon IS NOT NULL), --change this if necessary 0.3, 0.5, 0.7
...
SELECT
fk_sgplaces,
ST_BUFFER(polygon, equivalent_radius*(SQRT(0.5/coverage_target)-1)) AS polygon --change this if necessary 0.3, 0.5, 0.7
FROM
(SELECT
fk_sgplaces,
polygon,
