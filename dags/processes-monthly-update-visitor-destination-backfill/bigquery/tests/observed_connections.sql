DECLARE prev DEFAULT (SELECT FORMAT_DATE('%B', DATE_TRUNC(DATE_SUB( CAST("{{ md_date_start }}" AS DATE), INTERVAL 1 MONTH), MONTH)  ));
...
DECLARE the_date DATE DEFAULT "{{ md_date_start }}";
DECLARE the_warning BOOL  DEFAULT(FALSE);
DECLARE the_terminate BOOL  DEFAULT(FALSE);
DECLARE the_mon STRING DEFAULT (SELECT FORMAT_DATE('%B', DATE_TRUNC(DATE_SUB( CAST("{{ md_date_start }}" AS DATE), INTERVAL 0 MONTH), MONTH)  ));
