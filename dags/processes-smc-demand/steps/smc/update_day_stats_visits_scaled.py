...
def register(start: TaskInstance, dag: DAG) -> TaskInstance:
    ...
    create_day_stats_visits_scaled = OlvinBigQueryOperator(
        ...
        query="{% include './include/bigquery/smc/create_daily_stats_visits_scaled.sql' %}",
    )
    start >> create_day_stats_visits_scaled

    end_date = Variable.get("smc_demand_start_date")
    ...
    query_day_stats_visits_scaled = OlvinBigQueryOperator.partial(
        task_id="smc_query_day_stats_visits_scaled",
        query="{% include './include/bigquery/smc/daily_stats_visits_scaled.sql' %}",
    ...
    check_day_stats_visits_scaled_end = MarkSuccessOperator(
        task_id="smc_check_day_stats_visits_scaled_end",
    )
