...
def register(start: TaskInstance, dag: DAG) -> TaskGroup:
    ...
    for year_name in year_ranges_dict.keys():
        ...
        with TaskGroup(
            group_id=f"post_smc_poi_visits_scaled_daily_estimation_{year_name}_task_group"
        ) as group:
            ...
            query_poi_visits_scaled = OlvinBigQueryOperator.partial(
                task_id=f"post_smc_query_poi_visits_scaled_block_daily_estimation_{year_name}_{month}",
                query="{% include './include/bigquery/post_smc/poi_visits_scaling_block_daily_estimation.sql' %}",
                retries=3,
            ).expand(params=date_list_params)

            sleep_task = BashOperator(
                task_id=f"post_smc_cool_down_period_block_daily_estimation_{month}",
