CREATE OR REPLACE VIEW
    `{{ var.value.env_project }}.{{ params['validations_kpi_dataset'] }}.{{ params['kpi_v2_table_chart_full_date_range_view'] }}` AS
WITH
    avg_yoy_error AS (
        SELECT
            id,
    ...
    trend_place AS (
        ...
        FROM
            avg_yoy_error trend
            JOIN `{{ var.value.env_project }}.{{ params['validations_kpi_dataset'] }}.{{ params['dwm_correlations_table'] }}` t_corr ON trend.id = t_corr.id
