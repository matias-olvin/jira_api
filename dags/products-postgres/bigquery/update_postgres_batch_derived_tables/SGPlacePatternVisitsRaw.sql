create or replace table
    `{{ params['project'] }}.{{ params['postgres_batch_dataset'] }}.{{  params['SGPlacePatternVisitsRaw_table'] }}`
    -- `storage-dev-olvin-com.postgres_batch_dataset.SGPlacePatternVisitsRaw`
...
daily_visits as (
  ...
  FROM (
    ...
    FROM
    `{{ params['project'] }}.{{ params['postgres_batch_dataset'] }}.{{  params['SGPlaceDailyVisitsRaw_table'] }}`
--    `storage-prod-olvin-com.postgres_batch.SGPlaceDailyVisitsRaw`
    WHERE local_date = '{{ ds }}'
  )
  CROSS JOIN
...
hourly_visits as (
  ...
  FROM (
    SELECT
      ...
      JSON_EXTRACT_ARRAY(visits) AS visit_array, -- Get an array from the JSON string
    FROM
    `{{ params['project'] }}.{{ params['postgres_batch_dataset'] }}.{{  params['SGPlaceHourlyAllVisitsRaw_table'] }}`
    -- `storage-prod-olvin-com.postgres_batch.SGPlaceHourlyAllVisitsRaw`
