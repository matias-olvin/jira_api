...
with DAG(
    ...
    skip_placekeys = EmptyOperator(task_id="skip_placekeys")

    branch_task = BranchPythonOperator(
    task_id='branch_task',
    provide_context=True,
    python_callable=branch_task_fn,
    dag=dag
    )
    ...
    for feed in {"store_visits_feed", "store_visitors_feed", "store_visits_trend_feed"}:
        ...
        start >> sensor >> branch_task

    branch_task >> create_snapshot_places >> insert_placekeys_into_temp >> remove_placekeys_temp
    branch_task >> skip_placekeys >> end

    # data quality checks and export
    data_quality_start = EmptyOperator(task_id="data_quality_start")
    data_quality_end = EmptyOperator(task_id="data_quality_end")
    ...
    clear_placekeys_for_versioning = GCSDeleteObjectsOperator(
            ...
            prefix="type-2/{{ params['placekeys_table'] }}/{{ next_ds.replace('-', '/') }}/",
    )
    ...
    data_quality_end >> feed_export >> feed_export_finance
    
    feed_export_finance >> end