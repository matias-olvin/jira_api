...
CREATE SNAPSHOT TABLE IF NOT EXISTS `{{ var.value.env_project }}.{{ params['public_feeds_staging_dataset'] }}.{{ params['places_dataset'] }}-{{ params['places_history_table'] }}-{{ var.value.data_feed_places_version.replace('.', '-') }}`
CLONE `{{ var.value.env_project }}.{{ params['places_dataset'] }}.{{ params['places_history_table'] }}` OPTIONS(expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(),INTERVAL 90 DAY));
