...
def register(start: TaskInstance, dag: DAG) -> TaskInstance:
    create_day_stats_visits = OlvinBigQueryOperator(
        task_id="create_day_stats_visits",
        query="{% include './include/bigquery/daily_stats/create_day_stats_visits.sql' %}",
    )

    start >> create_day_stats_visits
    ...
    query_day_stats_visits_params = list()
    for timestamp_start, timestamp_end in zip(dates_compute[:-1], dates_compute[1:]):
        ...
        query_day_stats_visits_params.append(
            {"date_start": date_start, "date_end": date_end}
        )

    query_day_stats_visits = OlvinBigQueryOperator.partial(
        project_id="{{ var.value.dev_project_id }}",
        task_id="query_day_stats_visits",
        billing_tier="highest",
        query="{% include './include/bigquery/daily_stats/day_stats_visits.sql' %}",
    ).expand(params=query_day_stats_visits_params)

    create_day_stats_visits >> query_day_stats_visits

    day_stats_visits_end = MarkSuccessOperator(task_id="check_day_stats_visits_end")

    query_day_stats_visits >> day_stats_visits_end

    return day_stats_visits_end
