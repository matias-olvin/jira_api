...
task_id="delete_day_stats_visits",
        ...
        configuration={
            "query": {
                "query": "DELETE `{{ params['project'] }}.{{ params['metrics_dataset'] }}.{{ params['day_stats_visits_table'] }}` WHERE local_date = DATE_ADD( DATE('{{ ds }}'), INTERVAL -{{ var.value.latency_days_visits }} DAY )",
                "useLegacySql": "False",
            },
            "labels": {
                "pipeline": "{{ dag.dag_id }}",
...
task_id="delete_quality_ml_input",
        ...
        configuration={
            "query": {
                "query": "DELETE `{{ params['project'] }}.{{ params['quality_dataset'] }}.{{ params['model_input_complete_table'] }}` WHERE local_date = DATE_ADD( DATE('{{ ds }}'), INTERVAL -{{ var.value.latency_days_visits }} DAY )",
                "useLegacySql": "False",
            },
            "labels": {
                "pipeline": "{{ dag.dag_id }}",
...
task_id="delete_block_1",
        ...
        configuration={
            "query": {
                "query": "DROP TABLE IF EXISTS `{{ params['project'] }}.{{ params['poi_visits_staging_dataset'] }}.{{ params['block_1_output_table'] }}_{{ ds }}` ",
                "useLegacySql": "False",
            },
            "labels": {
                "pipeline": "{{ dag.dag_id }}",
...
task_id="delete_daily_estimation_factor",
        ...
        configuration={
            "query": {
                "query": "DELETE `{{ params['project'] }}.{{ params['daily_estimation_dataset'] }}.{{ params['daily_factor_table'] }}` WHERE local_date = DATE_ADD( DATE('{{ ds }}'), INTERVAL -{{ var.value.latency_days_visits }} DAY )",
                "useLegacySql": "False",
            },
            "labels": {
                "pipeline": "{{ dag.dag_id }}",
...
task_id="delete_daily_olvin",
        ...
        configuration={
            "query": {
                "query": "DELETE `{{ params['project'] }}.{{ params['daily_estimation_dataset'] }}.{{ params['grouped_daily_olvin_table'] }}` WHERE local_date = DATE_ADD( DATE('{{ ds }}'), INTERVAL -{{ var.value.latency_days_visits }} DAY )",
                "useLegacySql": "False",
            },
            "labels": {
                "pipeline": "{{ dag.dag_id }}",
...
task_id="delete_run",
        configuration={
            "query": {
                "query": f"""
                        DELETE FROM
                            `{{{{ params['project'] }}}}.{{{{ params['accessible_by_sns_dataset'] }}}}.{{{{ params['daily_estimation_runs_table'] }}}}`
                        WHERE
    ...
    trigger_daily_estimation = BashOperator(
        ...
        f'dags trigger -- -e "{{{{ ds }}}} {datetime.now().time()}" daily_estimation '
...
gcp_conn_id="google_cloud_olvin_default",
        sql=f"SELECT run_completion FROM `{dag.params['project']}."
...
task_id="load_daily_estimation_factor_from_sns",
    ...
    if mode == "update":
        # UPDATE TASKS
        delete_daily_stats_geohits = BigQueryInsertJobOperator(
            task_id="delete_daily_stats_geohits",
            project_id=dag.params["project"],
            configuration={
                "query": {
                    "query": "DELETE `{{ params['project'] }}.{{ params['metrics_dataset'] }}.{{ params['day_stats_geohits_table'] }}` WHERE local_date = DATE_ADD( DATE('{{ ds }}'), INTERVAL -{{ var.value.latency_days_visits }} DAY )",
                    "useLegacySql": "False",
                },
                "labels": {
                    "pipeline": "{{ dag.dag_id }}",
        ...
        query_daily_stats_geohits = BigQueryInsertJobOperator(
            task_id="query_daily_stats_geohits",
            project_id=dag.params["project"],
            configuration={
                "query": {
                    "query": "{% include './bigquery/places/scaling/daily_stats_geohits.sql' %}",
                    "useLegacySql": "False",
                    "destinationTable": {
                        "projectId": "{{ params['project'] }}",
                        "datasetId": "{{ params['metrics_dataset'] }}",
                        "tableId": "{{ params['day_stats_geohits_table'] }}",
                    },
                    "writeDisposition": "WRITE_APPEND",
                    "createDisposition": "CREATE_IF_NEEDED",
                },
        ...
        delete_daily_stats_clusters = BigQueryInsertJobOperator(
            task_id="delete_daily_stats_clusters",
            project_id=dag.params["project"],
            configuration={
                "query": {
                    "query": "DELETE `{{ params['project'] }}.{{ params['metrics_dataset'] }}.{{ params['day_stats_clusters_table'] }}` WHERE local_date = DATE_ADD( DATE('{{ ds }}'), INTERVAL -{{ var.value.latency_days_visits }} DAY )",
                    "useLegacySql": "False",
                },
                "labels": {
                    "pipeline": "{{ dag.dag_id }}",
        ...
        query_daily_stats_clusters = BigQueryInsertJobOperator(
            task_id="query_daily_stats_clusters",
            project_id=dag.params["project"],
            configuration={
                "query": {
                    "query": "{% include './bigquery/places/scaling/daily_stats_clusters.sql' %}",
                    "useLegacySql": "False",
                    "destinationTable": {
                        "projectId": "{{ params['project'] }}",
                        "datasetId": "{{ params['metrics_dataset'] }}",
                        "tableId": "{{ params['day_stats_clusters_table'] }}",
                    },
                    "writeDisposition": "WRITE_APPEND",
                    "createDisposition": "CREATE_IF_NEEDED",
                },
        ...
        (
            start
            >> delete_daily_stats_geohits
            >> query_daily_stats_geohits
            >> delete_daily_stats_clusters
            >> query_daily_stats_clusters
