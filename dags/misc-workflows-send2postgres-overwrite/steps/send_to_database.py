...
def register(dag, start, db):
    ...
    MACHINE_NAME="pg-db-send-overwrite"
    VM_ZONE = dag.params["vm_zone"]
    table=Variable.get("bigquery_batch_sent_table_name")
    final_tasks = []
    ...
    send_tables(
        dataset_bigquery=Variable.get("bigquery_batch_sent_dataset_name"),
        table_bigquery=Variable.get("bigquery_batch_sent_table_name"),
        database_table=Variable.get("postgres_batch_table_name"),
    )
