merge into
  `{{ var.value.env_project }}.{{ params['metrics_dataset'] }}.{{ params['groups_table'] }}_{{ dag_run.conf['step'] }}` all_metrics
...
using (
    with median_per_row as (
        select
            *,
            ...
            percentile_cont(cast({{ params.metric }}_abs as float64), 0.5) over(
                partition by
                    {{ params.group_id }},
                    start_date,
                    end_date,
        ...
        from
            `{{ var.value.env_project }}.{{ params['metrics_dataset'] }}.{{ params['pois_table'] }}_{{ dag_run.conf['step'] }}`, 
    ...
    join_results as (
        select
            ...
            any_value(median_abs) as median_abs,
            stddev(cast({{ params.metric }}_abs as float64)) as std_dev_abs,
            count(*) as total,
        from 
            median_per_row
        where
