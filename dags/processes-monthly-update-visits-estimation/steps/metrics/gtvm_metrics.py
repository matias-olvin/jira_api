...
task_id="clear_gtvm_visits_accuracy",
        configuration={
            "query": {
                "query": "{% include './bigquery/metrics/clear_gtvm_visits_accuracy.sql' %}",
                "useLegacySql": "false",
            }
        },
    )
    ...
    prev_task = clear_gtvm_visits_accuracy
    for metric in metric_options:
        if metric == "discrepancy":
            query = (
                "{% include './bigquery/metrics/update_gtvm_metric_discrepancy.sql' %}"
            )
        else:
            query = "{% include './bigquery/metrics/update_gtvm_metric.sql' %}"
        ...
        prev_task >> update_gtvm_metric
        prev_task = update_gtvm_metric

    prev_task >> update_gtvm_metric_end
