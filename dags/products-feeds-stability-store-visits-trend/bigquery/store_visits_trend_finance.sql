CREATE OR REPLACE TABLE `{{ var.value.env_project }}.{{ params['public_feeds_finance_dataset'] }}.{{ params['store_visits_trend_table'] }}`
PARTITION BY month_starting
CLUSTER BY store_id
AS

SELECT feed.*, SGBrandRaw.stock_symbol, SGBrandRaw.stock_exchange 
FROM `{{ var.value.env_project }}.{{ params['public_feeds_dataset'] }}.{{ params['store_visits_trend_table'] }}` feed
