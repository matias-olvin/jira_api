...
--  FROM `storage-prod-olvin-com.poi_visits_scaled.*`
  WHERE visit_score > 0
        AND local_date >= DATE_TRUNC(DATE_SUB( CAST("{{ ds.format('%Y-%m-%d') }}" AS DATE), INTERVAL 1 MONTH), MONTH)
        AND local_date < DATE_TRUNC(DATE_SUB( CAST("{{ ds.format('%Y-%m-%d') }}" AS DATE), INTERVAL 0 MONTH), MONTH)
),
