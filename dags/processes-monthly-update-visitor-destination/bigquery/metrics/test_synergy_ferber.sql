...
--    `{{ params['storage-prod'] }}.{{ params['visitor_brand_destinations_metrics_dataset'] }}.{{ params['monitoring_synergy_table']}}`
--AS
...
target_stores AS(
  ...
  WHERE region = 'FL' AND
        fk_sgbrands = 'SG_BRAND_42aefbae01d2dfd981f7da7d823d689e'
),
...
FROM `{{ params['storage-prod'] }}.{{ params['visitor_brand_destinations_metrics_dataset'] }}.{{ params['ferber_synergy_table'] }}`
-- `storage-prod-olvin-com.visitor_brand_destinations_metrics.ferber_synergy_ref`
)
...
SELECT DATE('{{ds}}') as run_date, *,
     ...
     OR IF(IF(factor_10>1.5,1,0) + IF(factor_20>1.5,1,0) + IF(factor_30>1.5,1,0) + IF(factor_50>2 ,1,0)>0, True, False) -- warn based on any factor over 1.5
     as warning,
     ...
     OR IF(IF(factor_10>1.5,1,0) + IF(factor_20>1.5,1,0) + IF(factor_30>2  ,1,0) + IF(factor_50>2  ,1,0)>2, True, False) -- warn based on >2 factors over 1.5
     OR IF(IF(factor_10>2  ,1,0) + IF(factor_20>2  ,1,0) + IF(factor_30>2.5  ,1,0) + IF(factor_50>2.5  ,1,0)>0, True, False) -- warn based on any factor over 2
    as terminate
FROM(
