        ...
        configuration={
            "load": {
                ...
                "destinationTable": {
                    ...
                    "tableId": "{{ params['neo4j_id_table'] }}_{{ execution_date.strftime('%Y%m%d') }}",
                },
        ...
        configuration={
            "load": {
                ...
                "destinationTable": {
                    "projectId": "{{ params['project'] }}",  # Variable.get("storage_project_id"),
                    "datasetId": "{{ params['networks_staging_dataset'] }}",
                    "tableId": "{{ params['predicted_edges_table'] }}_{{ execution_date.strftime('%Y%m%d') }}",
                },
                "writeDisposition": "WRITE_TRUNCATE",
                "createDisposition": "CREATE_IF_NEEDED",
                "schema": {
                    "fields": [
                        {"name": "src_node", "type": "INT64", "mode": "REQUIRED"},
                        {"name": "dst_node", "type": "INT64", "mode": "REQUIRED"},
...
task_id="mapping_id",
        configuration={
            "query": {
                ...
                "destinationTable": {
                    "projectId": "{{ params['project'] }}",  # Variable.get("storage_project_id"),
                    "datasetId": "{{ params['networks_staging_dataset'] }}",
                    "tableId": "{{ params['predicted_mapping_id_table'] }}_{{ execution_date.strftime('%Y%m%d') }}",
                },
                "writeDisposition": "WRITE_TRUNCATE",
                "createDisposition": "CREATE_IF_NEEDED",
            },
    ...
    #     configuration={
    #         "query": {
    #             "query": """DELETE `{{ params['project'] }}.{{ params['networks_dataset'] }}.{{ params['predicted_mapping_id_table'] }}` where local_date = '{{ ds.format('%Y-%m-01') }}'""",
    #             "useLegacySql": "False",
    #         },
    #         "labels": {
    #             "pipeline": "{{ dag.dag_id }}",
    ...
    #                         from `{{ params['project'] }}.{{ params['networks_staging_dataset'] }}.{{ params['predicted_mapping_id_table'] }}_{{ execution_date.strftime('%Y%m%d') }}`
    #                     """,
    #             "useLegacySql": "False",
    #             "destinationTable": {
...
task_id="transform_k_edges",
    ...
    #         "query": {
    #             "query": """DELETE
    #             `{{ params['project'] }}.{{ params['networks_metrics_dataset'] }}.{{ params['output_weight_vs_features_table'] }}` where local_date = '{{ ds.format('%Y-%m-01') }}'""",
    #             "useLegacySql": "False",
    #         },
    #         "labels": {
    #             "pipeline": "{{ dag.dag_id }}",
...
task_id="delete_historical_transform_k_edges",
        configuration={
            "query": {
                "query": """DELETE `{{ params['project'] }}.{{ params['networks_dataset'] }}.{{ params['predicted_mapping_id_table'] }}` where local_date = DATE('{{ ds.format('%Y-%m-01') }}')""",
                "useLegacySql": "False",
            },
            "labels": {
                "pipeline": "{{ dag.dag_id }}",
        ...
        configuration={
            "query": {
                "query": """
                            select *,
                                    DATE("{{ ds.format('%Y-%m-01') }}") AS local_date
                            from `{{ params['project'] }}.{{ params['networks_staging_dataset'] }}.{{ params['predicted_mapping_id_table'] }}_{{ ds_nodash.format('%Y%m01') }}`
                        """,
                "useLegacySql": "False",
                "destinationTable": {
                    "projectId": "{{ params['project'] }}",
...
task_id="query_second_deg_conn",
    ...
    wait_for_activity = ExternalTaskSensor(
        ...
        external_task_id="query_copy_final_Place_Activity",
        dag=dag,
        poke_interval=60 * 20,
...
task_id="query_postgres_table",
    ...
    #                            "{{ params['second_deg_conn_table'] }}${{ execution_date.strftime('%Y%m%d') }}",
    #     project_id="{{ params['project'] }}",
    #     dag=dag,
    # )
    #
    ...
    #                            "{{ params['mixed_conn_table'] }}${{ execution_date.strftime('%Y%m%d') }}",
    #     project_id="{{ params['project'] }}",
    #     dag=dag,
    # )
...
task_id="delete_historical_postgres_json",
        ...
        configuration={
            "query": {
                ...
                {{ params['postgres_connection_table'] }}` where local_date = IF ('{{params['mode']}}' = 'historical', PARSE_DATE('%F', '{{ ds.format('%Y-%m-01') }}'), DATE("{{ ds.format('%Y-%m-01') }}") )""",
                "useLegacySql": "False",
            },
            "labels": {
                "pipeline": "{{ dag.dag_id }}", 
...
task_id="append_to_historical_postgres_json",
    ...
    # mapping_id >> append_to_final_k_edges
    ...
    (
        ...
        >> output_end
    )  # >> query_smooth_conn >> query_postgres_table \
    # >> [delete_second_deg_conn, delete_mixed_conn] >> output_end

    # query_smooth_conn >> delete_monitoring_estimated >> sampling_for_monitoring_estimated \
    # >> append_to_monitoring_estimated
    # mapping_id >> delete_historical_mapping_id >> append_to_historical_mapping_id >> output_end
    # mapping_id >> output_end
