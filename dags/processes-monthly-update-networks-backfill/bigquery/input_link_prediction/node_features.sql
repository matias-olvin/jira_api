    ...
    FROM (
        SELECT
          IF (
              ...
              --DATE_TRUNC(DATE_SUB( CAST( PARSE_DATE('%F', "{{ ds.format('%Y-%m-01') }}") AS DATE), INTERVAL 0 MONTH), MONTH)
              ) AS result_date
    )
  ),
...
-- POI features
 places_with_coordinates_and_category AS (
  SELECT
    ...
    olvin_category
  FROM
    --`storage-dev-olvin-com.sg_places.20211101` c
    `{{ params['project'] }}.{{ params['places_dataset'] }}.{{ params['places_table'] }}` c
  LEFT JOIN
    --`storage-dev-olvin-com.sg_base_tables.naics_code_subcategories` m
...
-- DEVICE-ID | PERSONA
 demographics_based_on_zip_id AS (
    ...
    FROM
      --`storage-dev-olvin-com.static_demographics_data.zipcode_demographics_v2`
  ...
  FROM
    ...
    --`storage-dev-olvin-com.device_zipcodes.*`  where local_date = '2022-01-01'
  GROUP BY
    device_id ),
  device_personas AS (
  SELECT
...
CREATE OR REPLACE TABLE `{{ params['project'] }}.{{ params['networks_staging_dataset'] }}.{{ params['node_features_raw_table'] }}`
...
-- POI features
 places_with_coordinates_and_category AS (
  ...
  FROM
    --`storage-dev-olvin-com.sg_places.20211101` c
    `{{ params['project'] }}.{{ params['sg_places_dataset'] }}.{{ params['sg_places_table'] }}` c
  LEFT JOIN
    --`storage-dev-olvin-com.sg_base_tables.naics_code_subcategories` m
    `{{ params['project'] }}.{{ params['sg_base_tables_dataset'] }}.{{ params['non_sensitive_naics_codes_table'] }}` m
  USING
...
-- DEVICE-ID | PERSONA
 demographics_based_on_zip_id AS (
    ...
    FROM
      --`storage-dev-olvin-com.static_demographics_data.zipcode_demographics_v2`
      `{{ params['project'] }}.{{ params['static_demographics_dataset'] }}.{{ params['zipcode_demographics_v2_table'] }}`
      )
    WHERE rand = 1
    ),
  device_zipcodes AS (
  ...
  FROM
    `{{ params['project'] }}.{{ params['device_zipcodes_dataset'] }}.*`
    --`storage-dev-olvin-com.device_zipcodes.*`  where local_date = '2022-01-01'
    WHERE local_date >= "2021-11-01" AND local_date < "2022-08-01"
  GROUP BY
...
adding_visits_features as (
  SELECT
      ...
      IFNULL(sum(CASE WHEN time_of_day = 'evening' THEN visit_score END) / nullif(sum(visit_score),0), 0) as visits_evening,
  FROM poi_visits
  left join device_personas using (device_id, local_date)
  group by fk_sgplaces
