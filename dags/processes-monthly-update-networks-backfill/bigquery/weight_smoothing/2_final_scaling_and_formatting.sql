WITH adding_percentiles as (
    select
    *,
     rank_place_date/ GREATEST(count(*)  OVER ( PARTITION BY src_node, local_date), 1) AS percentile_weight
    from
      (SELECT *,
      RANK() OVER ( PARTITION BY src_node, local_date ORDER BY weight DESC) AS rank_place_date
      FROM
      `{{ params['project'] }}.{{ params['networks_staging_dataset'] }}.{{ params['second_deg_conn_table'] }}`)
      WHERE rank_place_date <= 200
),
...
(SELECT
  ...
  REPLACE(REPLACE(TO_JSON_STRING(ARRAY_AGG(STRUCT(CAST(dst_node AS STRING) AS dst_node,
      ...
      -- DATE_TRUNC(DATE_SUB( CAST("{{ ds.format('%Y-%m-01') }}" AS DATE), INTERVAL 0 MONTH), MONTH)
      -- ) AS local_date,
FROM
  estimated_edges
 WHERE total_node >= 20
...
(SELECT fk_sgplaces FROM 
  `{{ params['project'] }}.{{ params['postgres_dataset'] }}.{{ params['activity_table'] }}` WHERE
  activity IN ('active', 'watch_list', 'inactive') )
USING(fk_sgplaces)