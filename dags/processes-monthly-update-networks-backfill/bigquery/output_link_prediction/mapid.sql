with original_table as  (
    ...
    `{{ params['project'] }}.{{ params['networks_staging_dataset'] }}.{{ params['predicted_edges_table'] }}_{{ execution_date.strftime('%Y%m%d') }}`
),
...
inner join 
`{{ params['project'] }}.{{ params['networks_staging_dataset'] }}.{{ params['neo4j_id_table'] }}_{{ execution_date.strftime('%Y%m%d') }}`
