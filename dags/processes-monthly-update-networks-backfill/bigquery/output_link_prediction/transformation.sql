with original_table as (
    ...
    dst_node, 
    weight as weight
    --FROM `storage-dev-olvin-com.sg_networks_staging.predicted_chaintochain`
    FROM 
        `{{ params['project'] }}.{{ params['networks_staging_dataset'] }}.{{ params['predicted_mapping_id_table'] }}_{{ execution_date.strftime('%Y%m%d') }}`
),
    ...
    from
        (
            ...
            left join ( select fk_sgplaces, ST_GEOGPOINT(longitude, latitude) as point_1 from
                    -- `storage-dev-olvin-com.networks_staging.node_features_raw`
