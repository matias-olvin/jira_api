...
WHERE
    hour_ts IN (
        SELECT DISTINCT
            hour_ts
        FROM
            `{{ var.value.env_project }}.{{ params['forecast_weather_dataset'] }}.{{ params['hourly_grid_table'] }}_temp`
    )
    AND identifier IN (
        SELECT DISTINCT
...
SELECT
    ...
    factor
FROM
    `{{ var.value.env_project }}.{{ params['forecast_weather_dataset'] }}.{{ params['hourly_grid_table'] }}_temp` UNPIVOT(
        factor
        FOR identifier IN (
            precip_intensity,
            wind_speed,
