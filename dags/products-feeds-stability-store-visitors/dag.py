...
def branch_task_places_fn(**kwargs):
    if str(kwargs['execution_date'])>=Variable.get("smc_end_date_prev"):
        return 'insert_store_visitors_into_temp_increment'
    else:
...
def branch_task_insert_fn(**kwargs):
    if str(kwargs['execution_date'])>=Variable.get("smc_end_date_prev"):
        return 'data-feed-export-increment.insert_store_visitors'
    else:
        smc_end_date = Variable.get("smc_end_date_prev")
        exec_date = str(kwargs['execution_date'])
        interval=(datetime.strptime(smc_end_date, '%Y-%m-%d').date()-datetime.strptime(exec_date.split('T')[0], '%Y-%m-%d').date()).days
        if interval>24:
            ...
            return 'data-feed-export-backfill.insert_store_visitors'
...
with DAG(
    ...
    insert_store_visitors_into_temp_backfill = BigQueryInsertJobOperator(
        task_id="insert_store_visitors_into_temp_backfill",
        configuration={
            "query": {
                "query": "{% include './bigquery/store_visitors_temp_backfill.sql' %}",
                "useLegacySql": "false",
            }
        },
        params=dag.params,
    ...
    insert_store_visitors_into_temp_increment = BigQueryInsertJobOperator(
        task_id="insert_store_visitors_into_temp_increment",
        configuration={
            "query": {
                "query": "{% include './bigquery/store_visitors_temp_increment.sql' %}",
                "useLegacySql": "false",
            }
        },
        params=dag.params,
    ...
    create_snapshot_monthly_update >> branch_task_places >> set_version_places >> create_snapshot_places >> insert_store_visitors_into_temp_backfill
    create_snapshot_monthly_update >> branch_task_places >> insert_store_visitors_into_temp_increment

    [insert_store_visitors_into_temp_increment, insert_store_visitors_into_temp_backfill] >> data_quality_start
    ...
    DATAPROC_CLUSTER_NAME="pby-product-p-dpc-euwe1-stab-export-store-vstrs"
    ...
    with TaskGroup(group_id="data-feed-export-backfill") as feed_export_backfill:
        insert_store_visitors = BigQueryInsertJobOperator(
        task_id="insert_store_visitors",
        depends_on_past=False,
        configuration={
            "query": {
                "query": "{% include './bigquery/store_visitors_backfill.sql' %}",
                "useLegacySql": "false",
            }
        },
        params=dag.params,
        ...
        job={
            ...
            "pyspark_job": {
                ...
                "args": [
                    ...
                    f"--input_table={{{{ params['public_feeds_dataset'] }}}}.{{{{ params['store_visitors_table'] }}}}",
                    f"--output_folder=gs://{{{{ params['raw_feed_bucket'] }}}}/{{{{ params['store_visitors_table'].replace('_','-') }}}}",
                    f"--partition_column={PARTITION_COLUMN}",
                    f"--num_files_per_partition=10",
        ...
        insert_store_visitors >> clear_gcs >> submit_export_to_gcs_job
    ...
    with TaskGroup(group_id="data-feed-export-increment") as feed_export_increment:
        ...
        configuration={
            "query": {
                "query": "{% include './bigquery/store_visitors_increment.sql' %}",
                "useLegacySql": "false",
            }
        },
        params=dag.params,
        ...
        depends_on_past=False,
        bucket_name="{{ params['raw_feed_bucket'] }}",
        prefix=f"{{{{ params['store_visitors_table'].replace('_','-') }}}}/{PARTITION_COLUMN}={{{{ execution_date.strftime('%Y-%m') }}}}",
    )
    ...
    with TaskGroup(group_id="data-feed-export-finance-backfill") as feed_export_finance_backfill:
        ...
        configuration={
            "query": {
                "query": "{% include './bigquery/store_visitors_finance.sql' %}",
                "useLegacySql": "false",
            }
        },
        params=dag.params,
        ...
        job={
            ...
            "pyspark_job": {
                ...
                "args": [
                    f"--project={{{{ var.value.env_project }}}}",
                    f"--input_table={{{{ params['public_feeds_finance_dataset'] }}}}.{{{{ params['store_visitors_table'] }}}}",
                    f"--output_folder=gs://{{{{ params['raw_feed_finance_bucket'] }}}}/{{{{ params['store_visitors_table'].replace('_','-') }}}}",
                    f"--partition_column={PARTITION_COLUMN}",
                    f"--num_files_per_partition=10",
                    f"--append_mode=overwrite",
                    f"--date_start={BACKFILL_START}",
    ...
    clear_store_visitors_for_versioning_backfill = GCSDeleteObjectsOperator(
            task_id="clear_store_visitors_for_versioning_backfill",
            bucket_name="{{ params['versioning_bucket'] }}",
            prefix="type-2/{{ params['store_visitors_table'] }}/{{ next_ds.replace('-', '/') }}/",
    )

    export_store_visitors_for_versioning_backfill = OlvinBigQueryOperator(
        task_id="export_store_visitors_for_versioning_backfill",
        query="{% include './bigquery/export_store_visitors_for_versioning_backfill.sql' %}",
        billing_tier="high",
    )

    clear_store_visitors_for_versioning_increment = GCSDeleteObjectsOperator(
            task_id="clear_store_visitors_for_versioning_increment",
            bucket_name="{{ params['versioning_bucket'] }}",
            prefix="type-2/{{ params['store_visitors_table'] }}/{{ next_ds.replace('-', '/') }}/",
    )

    export_store_visitors_for_versioning_increment = OlvinBigQueryOperator(
        task_id="export_store_visitors_for_versioning_increment",
        query="{% include './bigquery/export_store_visitors_for_versioning_increment.sql' %}",
        billing_tier="high",
    )
    ...
    feed_export_increment >> clear_store_visitors_for_versioning_increment >> export_store_visitors_for_versioning_increment
    ...
    feed_export_backfill >> clear_store_visitors_for_versioning_backfill >> export_store_visitors_for_versioning_backfill
