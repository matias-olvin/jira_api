...
BEGIN CREATE TEMP TABLE batch_prediction AS (
  SELECT *
  FROM `{{ var.value.sns_project }}.{{ params['visits-estimation-supervised-dataset'] }}_{{ params['stage'] }}.{{ params['group-location-gt-scaled-prediction-table'] }}`
); END;

CREATE OR REPLACE TABLE `{{ var.value.sns_project }}.{{ params['visits-estimation-supervised-dataset'] }}_{{ params['stage'] }}.{{ params['rt-factors-tier-id-location-table'] }}` AS (
  ...
  , daily_gt AS (
    SELECT *
    FROM `{{ var.value.sns_project }}.{{ params['visits-estimation-supervised-dataset'] }}_{{ params['stage'] }}.{{ params['rt-gt-scaled-groups-location-table'] }}`
  )
  , data_prep AS (
    SELECT
      *
