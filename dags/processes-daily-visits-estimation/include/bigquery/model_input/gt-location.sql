...
BEGIN CREATE TEMP TABLE group_gt_scaled_table AS (
  ...
  FROM (
    ...
    INNER JOIN list_poi_good_latency
      USING (fk_sgplaces)
    INNER JOIN `{{ var.value.sns_project }}.{{ params['visits-estimation-supervised-dataset'] }}_{{ params['stage'] }}.{{ params['class-pois-sns-location-table'] }}`
      USING (fk_sgplaces)
    WHERE tier_id <> 'all'  -- all already done in location
    GROUP BY
      tier_id
      , local_date
  )
...
CREATE OR REPLACE TABLE `{{ var.value.sns_project }}.{{ params['visits-estimation-supervised-dataset'] }}_{{ params['stage'] }}.{{ params['rt-gt-scaled-groups-location-table'] }}`
AS (
  SELECT *
  FROM group_gt_scaled_table
  INNER JOIN (
