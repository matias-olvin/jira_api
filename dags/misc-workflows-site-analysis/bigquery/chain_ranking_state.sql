...
`{{ var.value.storage_project_id }}.{{ dag_run.conf['site_name'] }}.{{ params['chain_ranking_state_table'] }}` as

-- get pois we want to analyse
WITH 
 place_ids as (
SELECT
fk_sgplaces
...
and  region IN ( {{ dag_run.conf['region'] }} )

),
--join the visits - from ground truth volumne model
get_visits as (
