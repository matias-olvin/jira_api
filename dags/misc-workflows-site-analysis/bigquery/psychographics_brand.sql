  ...
  -- `storage-prod-olvin-com.lincoln_place_2.psychographics_brand` as
`{{ var.value.storage_project_id }}.{{ dag_run.conf['site_name'] }}.{{ params['psychographics_brand_table'] }}` AS
...
--       and local_date = '2021-01-01'
  ST_INTERSECTS(ST_BUFFER( {{ dag_run.conf['select_geometry_no_buffer'] }}, 160934), lat_long_visit_point)
   and local_date >= DATE_SUB(DATE_SUB(DATE_TRUNC(CURRENT_DATE(), MONTH), INTERVAL 1 DAY), INTERVAL 12 MONTH) and local_date <= DATE_SUB(DATE_TRUNC(CURRENT_DATE(), MONTH), INTERVAL 1 DAY)
    ...
    group_places as (
select fk_sgplaces, fk_sgbrands, device_id, count(fk_sgplaces) total_visits_to_poi , count(distinct device_id) unique_device_to_poi from get_pois_visited_by_device group by fk_sgplaces,fk_sgbrands, device_id
),
join_subcategory as (
...
group_brands_1 as (
select fk_sgbrands,  device_id, count(distinct device_id) unique_device_to_poi from get_pois_visited_by_device group by fk_sgbrands, device_id

),
group_brands_2 as (
select fk_sgbrands, count(distinct device_id) unique_device_to_poi from group_brands_1 group by fk_sgbrands
),

join_brands as (
select a.*, b.name, b.top_category, b.sub_category from group_brands_2 a
left join 
 `{{ var.value.storage_project_id }}.{{ params['places_dataset'] }}.{{ params['brands_table'] }}` b
-- `storage-prod-olvin-com.sg_places.brands` b
on a.fk_sgbrands = b.pid
order by unique_device_to_poi desc
)
  
select * from join_brands
