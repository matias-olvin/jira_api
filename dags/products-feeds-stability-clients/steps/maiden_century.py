    ...
    def branch_task_insert_fn(**context):
        ...
        if run_date.date() == first_monday.date():
            if run_date.date() == backfill_date.date():
                return "maiden_century.clear_gcs_placekeys_monthly"  # backfill
            else:
                return "maiden_century.clear_gcs_store_visitors_monthly_increment"  # increment
        else:
            return "maiden_century.end"  # end

    with TaskGroup(group_id="maiden_century") as group:

        DATAPROC_CLUSTER_NAME = "pby-product-p-dpc-euwe1-maiden-century-s3"
        ...
        clear_gcs_placekeys_monthly = GCSDeleteObjectsOperator(
            ...
            prefix="maiden_century/export_date={{ next_ds.replace('-', '') }}/{{ params['placekeys_table'] }}/",
        )

        clear_gcs_store_visitors_monthly_backfill = GCSDeleteObjectsOperator(
            task_id="clear_gcs_store_visitors_monthly_backfill",
            bucket_name="{{ params['feeds_staging_gcs_bucket'] }}",
            prefix="maiden_century/export_date={{ next_ds.replace('-', '') }}/{{ params['store_visitors_table'].replace('_','-') }}/",
        )
        ...
        clear_gcs_store_visits_trend_monthly_backfill = GCSDeleteObjectsOperator(
            ...
            bucket_name="{{ params['feeds_staging_gcs_bucket'] }}",
            prefix="maiden_century/export_date={{ next_ds.replace('-', '') }}/{{ params['store_visits_trend_table'].replace('_','-') }}/",
        )
        ...
        export_placekeys_monthly = OlvinBigQueryOperator(
            ...
            query="{% include './bigquery/maiden_century/placekeys_monthly.sql' %}",
        )

        export_store_visitors_monthly_backfill = OlvinBigQueryOperator(
            task_id="export_store_visitors_monthly_backfill",
            query="{% include './bigquery/maiden_century/store_visitors_monthly_backfill.sql' %}",
            billing_tier="high",
        )

        export_store_visits_trend_monthly_backfill = OlvinBigQueryOperator(
            task_id="export_store_visits_trend_monthly_backfill",
            query="{% include './bigquery/maiden_century/store_visits_trend_monthly_backfill.sql' %}",
        )

        export_store_visitors_monthly_increment = OlvinBigQueryOperator(
            task_id="export_store_visitors_monthly_increment",
            query="{% include './bigquery/maiden_century/store_visitors_monthly_increment.sql' %}",
            billing_tier="high",
        )

        export_store_visits_trend_monthly_increment = OlvinBigQueryOperator(
            task_id="export_store_visits_trend_monthly_increment",
            query="{% include './bigquery/maiden_century/store_visits_trend_monthly_increment.sql' %}",
        )

        source = "gs://{{ params['feeds_staging_gcs_bucket'] }}/maiden_century/export_date={{ next_ds.replace('-', '') }}"
        destination = "s3a://{{ params['maiden_century_bucket'] }}/store_visits/export_date={{ next_ds.replace('-', '') }}"
        ...
        create_export_to_s3_cluster = BashOperator(
            ...
            f"--initialization-actions 'gs://{dag.params['feeds_keys_gcs_bucket']}/pby-aws-send-maiden-century.sh' "
