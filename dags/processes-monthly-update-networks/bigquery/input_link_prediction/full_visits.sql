...
WITH
  ...
  visits_to_poi AS (
    ...
    --FROM `storage-prod-olvin-com.poi_visits_scaled.2022`
    FROM 
  ...
  --   FROM `{{ params['project'] }}.{{ params['device_visits_historical_dataset'] }}.{{ params['device_clusters'] }}`
  --   WHERE
  --     local_date >= (SELECT date_begin_data FROM date_range)
  --   AND 
  --     local_date <= (SELECT date_end_data FROM date_range)
  ...
  device_home_locations AS (
   ...
   SELECT 
    ...
    FROM 
       --`storage-prod-olvin-com.device_homes.2022`
  ...
  all_visits AS (
    ...
    --         DATE_TRUNC(DATE_SUB( CAST( PARSE_DATE('%F', "{{ execution_date.add(months=1).format('%Y-%m-01') }}") AS DATE), INTERVAL 1 MONTH), MONTH)
    --       ) AS local_date
    -- FROM 
