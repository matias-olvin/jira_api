...
def register(dag, start):
    ...
    MACHINE_NAME = f"{dag.params['networks_vm']}-{{{{ execution_date.add(months=1).format('%Y-%m-01') }}}}"
    ...
    import_to_neo4j = PythonOperator(
        ...
        templates_dict={
            "machine_name": MACHINE_NAME,
            "command": "sudo bash '/usr/bin/load_data.sh' '{{ execution_date.add(months=1).format('%Y%m01') }}' '' 'dbprod'",
        },
        provide_context=True,
        dag=dag,
    )
    ...
    export_edges_id = PythonOperator(
        ...
        templates_dict={
            "machine_name": MACHINE_NAME,
            "command": "sudo bash /usr/bin/run11.sh '{{ execution_date.add(months=1).format('%Y%m01') }}'",
        },
        provide_context=True,
        dag=dag,
    )
