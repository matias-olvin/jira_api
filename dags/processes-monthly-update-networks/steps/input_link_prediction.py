...
def create_header(dag, **context):
    ...
    connections_edges_header = [":START_ID", "weight:float", ":END_ID", ":TYPE"]
    ...
    execution_date = context["execution_date"].add(months=1).date().strftime("%Y%m01")
    sep_symbol = ","
    ...
    # with open('gs://neo4j-data-prod-olvin-com/{{ execution_date.add(months=1).format('%Y-%m-01') }}/observed_connections_edges_header.csv', 'w', newline='') as csvfile:

    #     csvwriter = csv.writer(csvfile, delimiter=',',
    #                         quotechar='|', quoting=csv.QUOTE_MINIMAL)
    #     csvwriter.writerow(connections_edges_header)

    # with open('gs://neo4j-data-prod-olvin-com/{{ execution_date.add(months=1).format('%Y-%m-01') }}/node_features_header.csv', 'w', newline='') as csvfile:
    #     csvwriter = csv.writer(csvfile, delimiter=',',
    #                         quotechar='|', quoting=csv.QUOTE_MINIMAL)
...
task_id="delete_historical_input_weight_vs_features",
        configuration={
            "query": {
                "query": """DELETE `{{ params['project'] }}.{{ params['networks_metrics_dataset'] }}.{{ params['input_weight_vs_features_table'] }}` where local_date = '{{ execution_date.add(months=1).format('%Y-%m-01') }}'""",
                "useLegacySql": "False",
            },
            "labels": {
                "pipeline": "{{ dag.dag_id }}",
...
task_id="append_to_historical_input_weight_vs_features",
    ...
    ## Deleting the same month
    delete_historical_observed_connections = BigQueryInsertJobOperator(
gcp_conn_id="google_cloud_olvin_default",        
task_id="delete_historical_observed_connections",
        configuration={
            "query": {
                "query": """DELETE `{{ params['project'] }}.{{ params['networks_dataset'] }}.{{ params['observed_connections_edges_table'] }}` where local_date = IF ('{{params['mode']}}' = 'historical', PARSE_DATE('%F', '{{ execution_date.add(months=1).format('%Y-%m-01') }}'), DATE("{{ execution_date.add(months=1).format('%Y-%m-01') }}") )""",
                "useLegacySql": "False",
            },
            "labels": {
                "pipeline": "{{ dag.dag_id }}",
    ...
    ## Appending
    append_to_historical_observed_connections = BigQueryInsertJobOperator(
gcp_conn_id="google_cloud_olvin_default",        
task_id="append_to_historical_observed_connections",
        configuration={
            "query": {
                "query": """
                            select *,
...
task_id="select_nodes_and_features",
    ...
    select_nodes_and_features >> query_connections_edges
...
task_id="export_connection_edges",
    ...
    start >> [query_full_visits, load_poi_representation]

    (
        query_full_visits
        >> query_connection_combinations
        >> query_connections_edges
        >> create_header_task
    )
    ...
    (
        query_connections_edges
        >> delete_historical_input_weight_vs_features
        >> append_to_historical_input_weight_vs_features
    )
    (
        query_connections_edges
        >> delete_historical_observed_connections
        >> append_to_historical_observed_connections
    )
