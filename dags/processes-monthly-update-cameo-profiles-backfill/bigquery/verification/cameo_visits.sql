CREATE OR REPLACE TABLE `{{ var.value.env_project }}.{{ params['cameo_staging_dataset'] }}.{{ params['cameo_visits_table'] }}_backfill_check` AS
WITH org_table_all AS (
  ...
  AND local_date < DATE_ADD("{{ params.date_start }}", INTERVAL 1 MONTH)
),
...
total_date_table_date AS (
  ...
  FULL OUTER JOIN (
    SELECT DATE_TRUNC("{{ params.date_start }}", MONTH) AS local_date
  )
  USING(local_date)
...
SELECT
*
FROM `{{ var.value.env_project }}.{{ params['cameo_staging_dataset'] }}.{{ params['cameo_visits_table'] }}_backfill_check`
 WHERE
 local_date >= DATE_SUB(DATE_TRUNC("{{ params.date_start }}", MONTH), INTERVAL 1 YEAR)
 AND(
 ...
 local_date < DATE_SUB(DATE_TRUNC("{{ params.date_start }}", MONTH), INTERVAL 1 YEAR)
 AND(
     IF(place_score_anomaly BETWEEN -2.25 AND 2.25, TRUE, ERROR("place score anomaly detected")) AND
     IF(place_cameo_anomaly > -2.25, TRUE, ERROR("place cameo anomaly detected")) AND
     IF(date_count_anomaly > -2.25, TRUE, ERROR("date count anomaly detected")) AND
     IF(date_place_anomaly > -1.5, TRUE, ERROR("date place anomaly detected")) AND
     IF(date_score_anomaly BETWEEN -1.8 AND 3, TRUE, ERROR("date score anomaly detected"))
 );
